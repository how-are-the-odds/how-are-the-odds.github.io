...
I have heard tell that school segregation is particularly bad in New York
schools, and a quick search seems to confirm that information. For instance,
...
segregation of Black students the second worse when it comes to Latino students.
I am going to look at the data [here](https://data.nysed.gov/downloads.php), to
see if,
...
There's a pretty rich supply of data on the website, to get started, I'm going
to look at the enrollment database for 2021-2022 (so only one year out of date),
since there is more rich accompanying data for that year (like graduation rates,
which I suspect that have yet to collect for 2023).
...
I see two main ways to do this. My first thought was to consider the _rank_ of
the resulting matrix, since if all the rows of the matrix are equal, then the
rank would be zero. Of course, we would want to avoid measuring the rank
directly, since these matrices are sufficiently generic that they should be full
rank. Thus, we want a more stable approximation of the rank. A classic example
is the nuclear norm, which forms a convex envelope of rank when
$\| E \|_{2} \le 1$. However, the math here is already getting far too
complicated for the use case, and I suspect that this approach will lack any
clear interpretability.

However, I have a second idea that I think will serve the problem better. Since
...
distracted by the overall baseline rates.

How do we solve this? Well, the matrices I generated care a lot about the
overall demographic makeup of the county. This is good to do to have an
expression for the lived experience of the "average", student. However, when
discussing school segregation, it is helpful to consider the segregation
_relative to_ what one would expect if there was no segregation at all. That is,
there is a baseline amount of exposure to a given demographic group that one
would expect if all schools in a county were exactly representative of the
overall demographic makeup. To do this, we will divide each column by the
expected proportion of the overall demographic makeup expected for that group.
This gives us these matrices:
...
unsegregated. In other words, $\tilde{E}_{AB}$ is the amount of representation
that of group $B$ that group $A$ experiences. So the result means that,
...
However, in the plot above, we can see a lot of grouping. Values in red
correspond to over-representation in one's class. So, for instance, in
Westchester County, we see that students that White, Asian, and Multiracial
students see overrepresentations of each other, as do Black, American Indian,
and Hispanic students. Between these two demographic "blocks", there is
generally under-representation. Perhaps more striking in Westchester county,
though, is that Black students are over-represented to other Black students by a
factor of three, while White and Black students see each other at 42% of the
expected rate.

I am trying hard not to take a moral/ethical stance on this, since it may be the
case that "segregation" is helpful for students in certain contexts. For
instance, in Onondaga county, the representation score for American Indians with
each other is larger than 20. In a vacuum, this means that American Indians see
twenty times as much of each other as they would if the county had one big
school. However, the alternative seems worse. In reality, only one fourth of the
average American Indian student's class is American Indian. In the
"unsegregated" situation, only about 1% of an American Indian's class would be
of the same demographic background. This honestly seems worse! So, particularly
when the overall number of students from a demographic group is small, the
representation stats might not be the most helpful tool.
